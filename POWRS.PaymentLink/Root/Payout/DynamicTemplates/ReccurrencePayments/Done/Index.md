Title: Payment Link
Description: Payment Link.
Date: 2023-08-04
Author: POWRS
Width: device-width
Cache-Control: max-age=0, no-cache, no-store
Pragma: no-cache
Expires: 0
CSS: index.cssx
Icon: favicon.ico
viewport : Width=device-width, initial-scale=1
Parameter: TokenId

<main class="border-radius">
<meta name="viewport" content="width=device-width, initial-scale=1" />
<div class="container">
<div class="content">
{{
buyerName:= "Nenad Aleksic";
amount:= 10.00;
productName:= "Naknada za usluge";
currency:= "RSD";
maxFailedAttempts:= 5;
sellerName:= "Test firma";
sellerPhoneNumber:= "+381638818943";
sellerEmail:= "nenad.aleksic@powrs.se";

]] 		<div class="header">
            <img src="data:image/png;base64," />
        </div>
        <div class="details">
            <div class="transaction-status">
                <h2>Your payment to ((sellerName )) failed</h2>			
                <p><strong>We were unable to process your last payment. This could have happened for several reasons:</strong></p>
                 <div class="reasons">
					<ul>
						<li>Your card has expired.</li>
						<li>Lack of funds or funds on hold.</li>
						<li>Your bank declined transaction for a reason unknown to us.</li>
					</ul>
				</div>
			<p class="line"></p>        
		</div>
			<div class="try-again">Don't you worry! We'll try to process your payment again in the next few days.</div>
			<p></p> 
			<div>To update your billing information and continue service without interruption, click here.</div> 
			<p></p> 
			<div class="update-div">
			   <a href="#" target="_blank" class="billing">Update my billing</a>
			</div>
			<p></p> 
			<p class="line"></p> 
        </div>     
        <div class="footer">
            <span class="footer-text"> POWRS D.O.O. BEOGRAD, Srbija</span>
            <span class="footer-text">©2021 - ((Now.Year)) POWRS </span>
            <span></span>
            <span style="text-align:left; margin-top:10px;" class="footer-text"> In accordance with Article 27 of the Law on Consumer Protection, you can fill out  <a href="https://pay.vaulter.rs/Obrazac-za-odustanak-od-ugovora.pdf ">contract withdrawal form</a></span>
        </div>
[[;
}}