Title: Payment Link
Description: Payment Link.
Date: 2023-08-04
Author: POWRS
Width: device-width
Cache-Control: max-age=0, no-cache, no-store
Pragma: no-cache
Expires: 0
CSS: index.cssx
Icon: favicon.ico
viewport : Width=device-width, initial-scale=1
Parameter: TokenId

<main style="border-radius: 8px;">
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <div style="width: 500px; margin: 0 auto; padding: 20px; background-color: #ffffff; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
    <div class="content">
    {{
    buyerName:= "Nenad Aleksic";
    amount:= 10.00;
    productName:= "Naknada za usluge";
    currency:= "RSD";
    maxFailedAttempts:= 5;
    sellerName:= "Test firma";
    sellerPhoneNumber:= "+381638818943";
    sellerEmail:= "nenad.aleksic@powrs.se";
    ]] <div style="text-align: center; padding: 10px;">
    <img src="data:image/png;base64," />
    </div>
    <div>
    <h2 style="color: red; border-bottom: 2px solid; padding-bottom: 5px;">Your payment to ((sellerName )) failed</h2>
    <p><strong>We were unable to process your last payment. This could have happened for several reasons:</strong></p>
    <div>
    <ul style="padding-left: 20px;">
    <li style="margin: 7px 0; list-style: disc;">Your card has expired.</li>
    <li style="margin: 7px 0; list-style: disc;">Lack of funds or funds on hold.</li>
    <li style="margin: 7px 0; list-style: disc;">Your bank declined the transaction for a reason unknown to us.</li>
    </ul>
    </div>
    <p style="border-bottom: 2px solid black;"></p> 
    <div style="padding-top: 10px;">Don't you worry! We'll try to process your payment again in the next few days.</div>
    <p></p>
    <div>To update your billing information and continue service without interruption, click here.</div>
    <p></p>
    <div style="display: flex; justify-content: center; align-items: center;">
    <a href="#" target="_blank" style="font-size: medium; padding: 5px 10px; margin-left: 8px; text-align: center; border-radius: 12px; font-weight: bold; line-height: normal; box-shadow: 0px 4px 5.5px 0px rgba(0, 0, 0, 0.1); transition: all 0.2s ease; font-family: Arial, sans-serif; cursor: pointer; color: white; background-color: black; text-decoration: none;">Update my billing</a>
    </div>
    <p></p>
    <p style="border-bottom: 2px solid black;"></p> 
    </div>
    <div style="color: #596273; font-family: Arial, Helvetica, sans-serif; font-size: 11px; font-style: italic; font-weight: 700; text-align: center;">
    <span style="display: block; text-align: center;">POWRS D.O.O. BEOGRAD, Srbija</span>
    <span style="display: block; text-align: center;">©2021 - ((Now.Year)) POWRS</span>
    <span></span>
    <span style="display: block; text-align: left; margin-top: 10px;">In accordance with Article 27 of the Law on Consumer Protection, you can fill out <a href="https://pay.vaulter.rs/Obrazac-za-odustanak-od-ugovora.pdf">contract withdrawal form</a></span>
    </div>
    [[;
    }}